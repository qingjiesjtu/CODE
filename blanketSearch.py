...
def blanketSearch(opts):
    name = get_name(opts, 'BS')
    print(name)
    ...
    samples_idx = shuffle  # create random poison samples idx

    for n in range(2): # always equal 2
        ...
        if n==1:
            ...
            train_data.data = train_data.data[samples_idx] \
                if opts.data_name in ('cifar10','cifar100') else [train_data.data[i] for i in samples_idx]
            train_data.targets = [train_data.targets[i] for i in samples_idx]            
        ...
        for epoch in range(opts.n_epoch):
            if opts.score in ('BAS','gradnorm') and epoch == opts.early_epoch and n<1: break
            if n==1: trigger.set_mode(0)
            elif opts.score  == 'gradnorm': trigger.set_mode(1)
            else: trigger.set_mode(2) 
            model.train() # poisoned training
            correct, total = 0, 0
            train_loss = 0
        ...
        if opts.score == 'BAS':
            trigger.set_mode(2), model.train() # poisoned training
            BAS, ds = [], []
        ...
        elif opts.score == 'gradnorm':
            trigger.set_mode(1), model.train()  
            gradnorms, ds = [], []
            train_loader = DataLoader(dataset=train_data, batch_size=1, shuffle=True,
                                      num_workers=2)  # batchsize=1 for compute score
            run_tqdm = tqdm.tqdm(train_loader, desc="Compute GraNd score")
            for x, y, b, s, d in run_tqdm:
                ...
                gradnorms.append(score.detach().cpu().numpy())  # record score of poisoned samples
                ds.append(d.detach().cpu().numpy())

                optimizer.zero_grad()

            gradnorms, ds = np.array(gradnorms), np.concatenate(ds, axis=0)
            gradnorms = gradnorms[np.argsort(ds)]  # from small to large
            gradnorms_idx = np.argsort(gradnorms)
            samples_idx = samples_idx[gradnorms_idx][::-1]  


        if n == 0:
            sample_dir = os.path.join(opts.sample_path)
            if not os.path.exists(sample_dir):
                os.makedirs(sample_dir)
            file_path = os.path.join(sample_dir, '{}.npy'.format(name))
            np.save(os.path.join(opts.sample_path, '{}.npy'.format(name)), samples_idx[:poison_num])  # save the selected poisoned samples

        samples_idx = samples_idx[:poison_num]  # retain a certain number of poisoned samples
